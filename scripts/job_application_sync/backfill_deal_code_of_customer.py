...
区別されない。** 実測した誤りは全てこれで説明がつく:
    ...
    星川産業 群馬営業所   → 本社の RL00000033 (同じ会社に本社契約が多数)
    シグマテック川越      → 茨城の RL00001207 (同一会社の2事業所が混在)
    タワー・メンテナンス   → 別法人(日本梱包運輸倉庫)の RL00001235
...
def plan_updates(nouhin: dict, links: dict, keijo: dict, now=None) -> dict:
    ...
    for did, p in nouhin.items():
        ...
        if own:
            ...
            #     星川産業 群馬営業所 → 本社の RL00000033
            #     シグマテック川越   → 茨城の RL00001207
            #     タワー・メンテナンス → 別法人(日本梱包運輸倉庫)の RL00001235
